...
//	}
	...
	public void openDriver(SurfaceHolder holder) throws IOException {
		...
		for (int camIdx = 0; camIdx < cameraCount; camIdx++) {
			...
			if(BaseApplication.CamerDirection == 1) {
				//打开前置摄像头
...
//	}
	...
	public Rect getFramingRect() {
		Point screenResolution = configManager.getScreenResolution();
		int min_frame_width = Dp2Px(context, 180);
		int min_frame_height = Dp2Px(context,180);
		int max_frame_width = Dp2Px(context,280);
		int max_frame_height = Dp2Px(context,240);
		if (framingRect == null) {
			if (camera == null) {
				return null;
			}
